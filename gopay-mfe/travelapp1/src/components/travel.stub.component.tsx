...
export default function TravelStub(props) {
  ...
  let check = useSelector((state) =>
    props.way === "return" ? state.travel.checked[1] : state.travel.checked[0]
  );
      ...
      {flights.map((value, index) => {
        ...
        return (
          <ListItem key={index} button>
            <ListItemAvatar>
              <Avatar alt={`Avatar n°${index + 1}`} src={`spicejet.png`} />
            </ListItemAvatar>
            <ListItemText
              id={labelId}
              primary={getString(value)}
              secondary="SpiceJet"
            />
            <ListItemSecondaryAction>
              <Checkbox
                ...
                checked={check ? check.indexOf(index) !== -1 : false}
