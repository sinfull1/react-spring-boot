...
export default function Step1(props) {
    ...
    const handleOriginToggle = (value) => () => {
        dispatch({ type: "SET_ORIGIN_FLIGHT", payload: { checked: [value] } })
    };

    const handleDestinationToggle = (value) => () => {
        dispatch({ type: "SET_DESTINATION_FLIGHT", payload: { checked: [value] } })
    };
    ...
    return (
        ...
        <div className="step-1-group">
            ...
            {toggle ? <div className="step-1-checkgroup">
                <TravelStub way="one" show={toggle} toggle={handleOriginToggle}></TravelStub>
                <TravelStub way="return" show={toggle} toggle={handleDestinationToggle}></TravelStub>
                <TravelSummary toggle={toggle} flights={getFlights()} check={check}></TravelSummary>
            </div> : null}
