...
let sseEvents = new EventSource("https://www.gopaychain.com:8443/getHttp?subsId=" + id);
sseEvents.onopen = event => console.log('open', event);
sseEvents.onerror = event => {
  console.log("Server side shut");
  sseEvents.close();
  sseEvents = new EventSource("https://www.gopaychain.com:8443/getHttp?subsId=" + id);
}
const savedFilter = [];
...
export default function Events(props) {
  ...
  function addRowData(newData) {
    fetch('https://www.gopaychain.com:8443/event?subsId=' + id + "&event=sample");
  };
